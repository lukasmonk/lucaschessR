...
class WindowTutor(QTVarios.WDialogo):
    def __init__(self, manager, tutor, siRival, siOpenings, is_white, siPuntos):
        ...
        if siRival:
            ...
            layout.controlc(gbOpenings, fr, cr).controlc(self.boardOpening, fr + 1, cr).otro(
                lytbOpening, fr + 2, cr
            )
...
def cambioTutor(parent, configuration):
    ...
    if resultado:
        ...
        configuration.tutor = configuration.buscaTutor(claveMotor)
        configuration.x_tutor_mstime = int(vtime * 1000)
        configuration.x_tutor_depth = depth
        configuration.x_tutor_multipv = multiPV
        configuration.x_tutor_difpoints = difpts
        configuration.x_tutor_difporc = difporc
        configuration.graba()
        return True
    else:
        return False
