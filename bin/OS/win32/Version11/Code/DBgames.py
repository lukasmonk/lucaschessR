...
class DBgames:
    ...
    def yield_todas(self, desde):
        select = ",".join(self.liCamposAll)
        self._cursor.execute("SELECT %s FROM %s" % (select, self.tabla))
        for n, raw in enumerate(self._cursor.fetchall()):
            if n >= desde:
                yield self.leePartidaRaw(raw)
        yield None
